#!/bin/bash
# Q8_E_BT.c (hasGreatGrandchild) 자동 테스트
# 사용법: ./q8_test.sh        (Q8_E_BT.c 와 같은 폴더에 두고 실행)
#         DEBUG=1 ./q8_test.sh   (실패 시 프로그램 원본 출력까지 보기)
#
...
SRC="Q8_E_BT.c"
PAT='s/.*at least one great-grandchild are: //p'

bin=$(mktemp /tmp/q8bt.XXXXXX) || exit 1
trap 'rm -f "$bin"' EXIT
...
echo
check "PDF 예시 (50만 해당)" "50 30 60 25 65 a a 20 a a a 10 75 a a a 15 a a" "50"
check "깊이 4 사슬" "1 2 a 3 a 4 a a a" "1"
check "깊이 2 완전트리 (없음)" "5 3 7 1 2 a a a a 4 8 a a a a" ""
check "bal7 (없음)" "4 2 6 1 3 a a a a 5 7 a a a a" ""
check "단일 노드" "42 a a" ""
check "빈 트리" "a" ""
