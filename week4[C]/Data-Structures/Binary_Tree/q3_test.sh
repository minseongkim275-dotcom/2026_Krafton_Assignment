#!/bin/bash
# Q3_E_BT.c (countOneChildNodes) 자동 테스트
# 사용법: ./q3_test.sh        (Q3_E_BT.c 와 같은 폴더에 두고 실행)
...
SRC="Q3_E_BT.c"
PAT='s/.*exactly one child node is: \([0-9-]*\)\..*/\1/p'

bin=$(mktemp /tmp/q3bt.XXXXXX) || exit 1
trap 'rm -f "$bin"' EXIT
...
echo
check "PDF 예시" "50 20 60 10 30 a a 55 a a a a 80 a a" "2"
check "완전 트리 (0개)" "5 3 7 1 2 a a a a 4 8 a a a a" "0"
check "증손자 트리" "50 30 60 25 65 a a 20 a a a 10 75 a a a 15 a a" "2"
check "왼쪽 사슬" "1 2 a 3 a a a" "2"
check "오른쪽 사슬" "1 a 2 a 3 a a" "2"
check "노드 2개" "10 5 a a a" "1"
check "mirror 트리" "4 5 2 a 6 a a 3 1 a a a a" "1"
