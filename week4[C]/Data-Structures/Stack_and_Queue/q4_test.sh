#!/bin/bash
# Q4_C_SQ.c (reverse) 자동 테스트
# 사용법: ./q4_test.sh        (Q4_C_SQ.c 와 같은 폴더에 두고 실행)
...
# "The resulting queue after reversing its elements is: 5 4 3 2 1"
PAT='s/.*reversing its elements is: //p'

bin=$(mktemp /tmp/q4sqtest.XXXXXX) || exit 1
trap 'rm -f "$bin"' EXIT
